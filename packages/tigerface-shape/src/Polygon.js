...
class Polygon extends Shape {
    ...
    _containsByBoundingRect(polygon) {
        ...
        const s1 = new Point(rect1.left, rect1.top);
        const e1 = new Point(rect1.left + rect1.width, rect1.top + rect1.height);

        const rect2 = polygon.getBoundingRect();
        const s2 = new Point(rect2.left, rect2.top);
        const e2 = new Point(rect2.left + rect2.width, rect2.top + rect2.height);
    ...
    contains(polygon) {
        // 判断外接矩形是否包含
        const success = this._containsByBoundingRect(polygon);
    ...
    hitTestPoint(point) {
        ...
        // 如果点就是顶点，那么直接返回 true
        for (let i = 0; i < this.getVertexes().length; i += 1) {
        ...
        for (let i = 1; i <= this.getVertexes().length; i += 1) {
            ...
            if (point.y > Math.min(p1.y, p2.y) && point.y < Math.max(p1.y, p2.y)) {
                ...
                if (p1.x === p2.x) {
                    ...
                    const x0 = (point.y - p1.y) * (p2.x - p1.x) / (p2.y - p1.y) + p1.x;
                ...
                // 下一顶点
                const p3 = this.getVertexes()[(i + 1) % this.getVertexes().length];
    ...
    hitTestPolygon(polygon) {
        ...
        // 与矩形的碰撞检测
        // console.log("hit", this.clazzName, polygon.clazzName);
        if (this.clazzName === 'Rectangle' && polygon.clazzName === 'Rectangle') {
            // console.log("hit", "Rectangle");
            return this.hitTestRectangle(polygon);
        }

        // 检测边碰撞
        const sides2 = polygon.getSides();
        for (let i = 0; i < sides2.length; i += 1) {
            const side = sides2[i];
        ...
        // 检测点边碰撞
        const points1 = this.getVertexes();
        ...
        // 反向检测点边碰撞
        const points2 = polygon.getVertexes();
        const sides1 = this.getSides();
    ...
    convertRounded(radius, precision = 10) {
        ...
        if (rp2) {
            const curvePoints = new QuadraticBezier(rp2, p1, rp0, precision).getPoints();
            newPoints = newPoints.concat(curvePoints);
        }
    ...
    getIntersectionOfLine(line) {
        ...
        for (const i in sides) {
            const p = sides[i].getIntersection(line);
            if (p) {
                ips.push({ side: sides[i], point: p });
            }
        }
        // 按与 line 起点的距离排序交点
        ips.sort((a, b) => a.point.getDistance(line.p0) - b.point.getDistance(line.p0));
    ...
    merge(polygon, g) {
        ...
        const record = (p) => {
            ...
            if (nps.length > 0) {
                ...
                const l = new Line(nps[nps.length - 1], v);
        ...
        // 先得到中点
        const rect2 = s2.getBoundingRect();
        const pc2 = new Point(rect2.left + rect2.width / 2, rect2.top + rect2.height / 2);
        // 寻找最远未碰撞点
        let p;
        for (let i = 0; i < s1.getVertexes().length; i += 1) {
        ...
        // 如果没找到，反过来找
        if (!p) {
            const rect1 = s1.getBoundingRect();
            const pc1 = new Point(rect1.left + rect1.width / 2, rect1.top + rect1.height / 2);
            for (let i = 0; i < s2.getVertexes().length; i += 1) {
        ...
        if (p) {
            ...
            const tot = s1.getVertexes().length * s2.getVertexes().length;
            // tot = 26;
            let t = 0;
            do {
                // 每次进入循环的点，一定是外部点，先记录
                ...
                // 判断交点
                const ips = (p.owner === s1 ? s2 : s1).getIntersectionOfLine(p.getSide());
                ...
                if (!ignore && ips && ips.length > 0) { // 有交点
                    ...
                    if (p.next.equals(ips[0].point)) {
                        ...
                        // ns 下一边，is 交点边
                        const ps = T.radianToDegree(new Line(ips[0].point, p).getSlope());
